...
app.get('/list_count', (req, res) => {
  const getUserCountQuery = `SELECT * FROM list_1`
  pool.query(getUserCountQuery, (error,result) => {
    if (error) res.end(error)
    var results = result.rows.length
    res.send(`${results}`)
...
app.get('/task_count', (req, res) => {
  const getUserCountQuery = `SELECT * FROM list_1 where id='2'`
  pool.query(getUserCountQuery, (error,result) => {
    if (error) res.end(error)
    var results = result.rows[0].tasks.length
    res.send(`${results}`)
  })
})

app.get('/task_complete', (req, res) => {
  const getUserCountQuery = `SELECT * FROM list_1 where id='2'`
  pool.query(getUserCountQuery, (error,result) => {
    if (error) res.end(error)
    var results = result.rows[0].tasks[0].complete
    res.send(`${results}`)
  })
})
...
app.post('/del_user', (req,res) => {
  ...
  pool.query(dropUserTableQuery, (error,result) => {
    if (error) res.end(error)
    console.log('dropped user list table')
  })
